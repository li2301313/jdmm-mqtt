...
class phpMQTT
{
    ...
    private $msgid = 1;            /* counter for message id 消息id计数*/
    public $keepalive;        /* default keepalive timmer 默认的心跳时间*/
    public $timesinceping;        /* host unix time, used to detect disconects 主机unix时间，用于检测断开连接*/
    public $topics = array();    /* used to store currently subscribed topics 存储当前订阅的主题*/
    public $debug;        /* should output debug messages 是否打印debug信息*/
    public $address;            /* broker address 消息服务器地址*/
    public $port;                /* broker port 消息服务器端口*/
    public $clientid;            /* client id sent to brocker 发送给brocker的客户端id*/
    public $will;                /* stores the will of the client 存储客户的will*/
    private $username;            /* stores username */
    private $password;            /* stores password */
    public $cafile;

    function __construct($address, $port, $clientid, $keepalive= 10, $debug = false, $cafile = NULL)
    {
        $this->broker($address, $port, $clientid, $keepalive, $debug, $cafile);
    }

    /* sets the broker details */
    function broker($address, $port, $clientid, $keepalive , $debug, $cafile = NULL)
    {
